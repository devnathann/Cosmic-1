...
class ErrorBag
{
    ...
    public function firstOfAll(string $format = ':message', bool $dotNotation = false): array
    {
        ...
        foreach ($messages as $key => $keyMessages) {
            if ($dotNotation) {
                $results = Flash::addMessage($results[$key] . $this->formatMessage(array_shift($messages[$key]), $format));
            } else {
                echo '{"status":"error","message":"' . $this->formatMessage(array_shift($messages[$key]), $format) . '"}';
                exit;
            }
        }
        return $results;
