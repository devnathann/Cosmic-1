...
class Forum
{
    ...
    { 
        $forums = Forums::getCategory(request()->player->id);
    ...
    {  
        $forums = Forums::getGuilds($this->slug($slug), request()->player->id);
        $forums->slug = Core::convertSlug($forums->name);
    ...
    public function topic($slug, $page = 1)
    {
        ...
        $forum = Forums::getGuilds($topic->guild_id);
    ...
    public function reply()
    {
        ...
        $reply_id = Forums::createReply($topic_id, Core::FilterString(Core::tagByUser($message)), request()->player->id); 
        echo '{"status":"success","message":"' . Locale::get('core/notification/message_placed') . '","replacepage":"forum/thread/' . $topic->id . '-'. Core::convertSlug($topic->subject) . '/page/'. $totalPages . '#' . $reply_id . '"}';
    }
    ...
    public function userlevel($guild_id) {
        return Forums::getGuildLevel($this->slug($guild_id), request()->player->id) ?? $this->state["EVERYONE"];
    }
