...
class Marketplace
{
    ...
    public function sell()
    {
        ...
        if(!Value::ifCurrencyExists($currency)) {
            response()->json(["status" => "error", "message" => "Currency does not exist"]);
        }
      
        if(!Value::ifItemExists($item_id, request()->player->id)) {
            response()->json(["status" => "error", "message" => "Item does not exists"]);
        }
      
        Value::sellItem($item_id, request()->player->id, $currency, $costs);       
        response()->json(["status" => "success", "message" => "Bedankt voor je aankoop blabla", "replacepage" => "marketplace/my/inventory"]);
    }
    ...
    public function catalogue()
    {
        ...
        {
            response()->json(["status" => "error", "message" => "Is not implementated yet."]);
            ...
            if($item->timestamp_expire < time()) {
                ...
                response()->json(["status" => "error", "message" => Locale::get('shop/marketplace/expired'), "replacepage" => "marketplace/my/inventory"]);
            }
            ...
            response()->json(["status" => "success", "message" => Locale::get('shop/marketplace/purchased'), "replacepage" => "marketplace/all/sell"]);
        }
