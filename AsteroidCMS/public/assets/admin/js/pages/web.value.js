    ...
    return {
        ...
        getItem: function(id) {
            ...
            self.ajax_manager.post("/housekeeping/api/value/getValueById", {post: id}, function (result) {
                ...
                $("#viewRareValue [name=points_type]").val(result.value.points_type);
                $("#viewRareValue [name=club_only]").val(result.value.club_only);
              
                for (var index in result.currencys){
                    console.log(result.currencys[index])
                    $("[name=parent_id]").append(new Option(currencys.caption, parent_page.id));
                }
...
jQuery(document).ready(function() {
    alert('This will be added later in a commit!');
    return;
    value.init();
});